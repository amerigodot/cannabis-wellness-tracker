...
serve(async (req) => {
  ...
  try {
    ...
    const ipRateLimitCheck = await checkIPRateLimit(supabaseAdmin, clientIP, "report");
    ...
    const userRateLimitCheck = await checkUserRateLimit(supabaseAdmin, user.id, "report");
