...
## 1. Executive Summary
This project redefines **Medical AI Privacy** by moving the entire clinical decision support pipeline to the **Edge**. We leverage **Google's Gemma-2B** model running directly in the browser via WebGPU to provide HIPAA-compliant harm reduction coaching without a single byte of patient data leaving the device.
...
*   **Clinical RAG on the Edge:** We embedded a structured JSON knowledge base of 2025 Clinical Guidelines (ACOEM, NCSCT) that the local model queries in real-time to ground its advice.
